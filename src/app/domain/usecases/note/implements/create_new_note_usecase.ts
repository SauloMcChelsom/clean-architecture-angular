...
import { CreateNewNoteUseCase } from '../note_usecase';
import { TransformTitleTo } from 'src/app/domain/helpers/transform/transform_title_to';

@Injectable({ providedIn: 'root' })
export class CreateNewNoteUseCaseImp implements CreateNewNoteUseCase {
    ...
    public createNewNote(note: NoteEntity): Observable<NoteEntity> {
        ...
        const createNewNote = of([]).pipe(switchMap(() => this._note.createNewNote(note)));
        const findByTitleNote = of([]).pipe(
            switchMap(() => this._note.findByTitleNote(note.title).pipe(
                switchMap((note) => {
                    if (note != undefined) {
