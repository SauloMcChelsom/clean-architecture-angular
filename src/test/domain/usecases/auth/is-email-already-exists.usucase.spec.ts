...
import { ICreateNewAccountUseCase, IIsEmailAlreadyExistsUseCase, ISignInWithEmailAndPassworUseCase } from 'src/app/domain/usecases/auth/auth_usecase';
import { CreateNewAccountUseCaseImp } from 'src/app/domain/usecases/auth/implements/create_new_account_usecase_imp';
import { IsEmailAlreadyExistsUseCaseImp } from 'src/app/domain/usecases/auth/implements/is_email_already_exists_usucase_imp';
...
describe('[IsEmailAlreadyExists]', () => {
    let service: IIsEmailAlreadyExistsUseCase;
    let repository: AuthenticationRepositoryImp;
    beforeEach(() => {
        TestBed.configureTestingModule({
            providers: [
                {
                    provide: IIsEmailAlreadyExistsUseCase,
                    useClass: IsEmailAlreadyExistsUseCaseImp,
                },
                {
        ...
        service = TestBed.inject(IIsEmailAlreadyExistsUseCase);
        repository = TestBed.inject(AuthenticationRepositoryImp);
    });

    it('should is authenticated with successfully', () => {
        return forkJoin({
            ...
            'isEmailAlreadyExists': service.isEmailAlreadyExists('ana@gmail.com.br'),
        }).pipe(
            catchError((error) => {
            ...
            error: (error) => {
                console.error('isEmailAlreadyExists', error);
                expect(error).toBeUndefined();
            }
        });
    });

    it('should return error for non-standard email', () => {
        service.isEmailAlreadyExists('anagmail.com.br').subscribe({
            next: () => { },
            error: (error) => {
                expect(error).toBe('Este e-mail não é valido');
            }
        });
    });
});